  ...
  fetch('calendar_data.json')
    ...
    .then(data => {
      ...
      if (todayEntry) {
        ...
        const anahulu = todayEntry.anahulu.trim();

        // Normalize function: removes okina and macrons
        function normalizeName(name) {
          return name
            .replace(/[ʻ‘’]/g, '')  // remove okina, quotes
            .replace(/ū/g, 'u')      // macron letters to simple vowels
            .replace(/ā/g, 'a')
            .replace(/ē/g, 'e')
            .replace(/ī/g, 'i')
            .replace(/ō/g, 'o');
        ...
        if (anahulu.includes('hō‘emi') && repeats.includes(normalizedName)) {
          imageName += '2';
        }

        const moonImage = document.getElementById('moon-phase-image');
        moonImage.src = `MoonPhaseIMG/${imageName}.png`;
        moonImage.alt = imageName;
